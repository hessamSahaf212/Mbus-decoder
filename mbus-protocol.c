/**
 * special thanks to Robert Johansson and contributors, Raditex AB
 *  Contributors:
 * Large parts of this file was contributed by Tomas Menzl
 *
 * Mbus Telegram decoder to XML and json format
 */
...
//------------------------------------------------------------------------------
mbus_frame *
mbus_frame_new(int frame_type)////////                              to encode
{
    ...
    switch (frame_data->type)
    {
        ...
        case MBUS_DATA_TYPE_VARIABLE:
            ...
            for (record = frame_data->data_var.record; record; record = record->next)
            {
                // pack DIF
                if (parse_debug)
                    printf("%s: packing DIF [%zu]", __FUNCSIG__, frame->data_size);
                ...
                // pack VIF
                if (parse_debug)
                    printf("%s: packing VIF [%zu]", __FUNCSIG__, frame->data_size);
                ...
                // pack data
                if (parse_debug) 
                    printf("%s: packing data [%zu : %zu]", __FUNCSIG__, frame->data_size, record->data_len);
                for (j = 0; j < record->data_len; j++)
                {
...
mbus_frame_print(mbus_frame *frame)
{
    ...
    for (iter = frame; iter; iter = iter->next)
    {
        ...
        printf("%s: Dumping M-Bus frame [type %d, %d bytes]: ", __FUNCSIG__, iter->type, len);
        for (i = 0; i < len; i++)
        {
...
mbus_data_variable_header_print(mbus_data_variable_header *header)
{
    if (header)
    {
        ...
        printf("%s: Version      = 0x%.2X\n", __FUNCSIG__, header->version);
        printf("%s: Medium       = %s (0x%.2X)\n", __FUNCSIG__, mbus_data_variable_medium_lookup(header->medium), header->medium);
        printf("%s: Access #     = 0x%.2X\n", __FUNCSIG__, header->access_no);
        printf("%s: Status       = 0x%.2X\n", __FUNCSIG__, header->status);
...
mbus_data_variable_print(mbus_data_variable *data)
{
    ...
    {   
        ...
        for (record = data->record; record; record = record->next)
        {
            ...
            {
                printf("%s: VENDOR DATA [size=%zd] = ", __FUNCSIG__, record->data_len);
                for (j = 0; j < record->data_len; j++)
                {
...
mbus_data_fixed_print(mbus_data_fixed *data)
{
    if (data)
    {    
        printf("%s: ID       = %d\n", __FUNCSIG__, (int)mbus_data_bcd_decode(data->id_bcd, 4));
        printf("%s: Access # = 0x%.2X\n", __FUNCSIG__, data->tx_cnt);
        printf("%s: Status   = 0x%.2X\n", __FUNCSIG__, data->status);
        printf("%s: Function = %s\n", __FUNCSIG__, mbus_data_fixed_function(data->status));
        
        printf("%s: Medium1  = %s\n", __FUNCSIG__, mbus_data_fixed_medium(data));
        printf("%s: Unit1    = %s\n", __FUNCSIG__, mbus_data_fixed_unit(data->cnt1_type));
        if ((data->status & MBUS_DATA_FIXED_STATUS_FORMAT_MASK) == MBUS_DATA_FIXED_STATUS_FORMAT_BCD)
        {
            printf("%s: Counter1 = %d\n", __FUNCSIG__, (int)mbus_data_bcd_decode(data->cnt1_val, 4));
        }
        else
        {
            printf("%s: Counter1 = %d\n", __FUNCSIG__, mbus_data_int_decode(data->cnt1_val, 4));
        }

        printf("%s: Medium2  = %s\n", __FUNCSIG__, mbus_data_fixed_medium(data));
        printf("%s: Unit2    = %s\n", __FUNCSIG__, mbus_data_fixed_unit(data->cnt2_type));
        if ((data->status & MBUS_DATA_FIXED_STATUS_FORMAT_MASK) == MBUS_DATA_FIXED_STATUS_FORMAT_BCD)
        {
            printf("%s: Counter2 = %d\n", __FUNCSIG__, (int)mbus_data_bcd_decode(data->cnt2_val, 4));
        }
        else
        {
            printf("%s: Counter2 = %d\n", __FUNCSIG__, mbus_data_int_decode(data->cnt2_val, 4));        
        }          
    }
...
mbus_data_variable_header_jSon(mbus_data_variable_header* header)
{
    ...
    if (header)
    {
        ...
        len += snprintf(&buff[len], sizeof(buff) - len, "        \"I\":  %d\n", val);
...
mbus_data_variable_record_xml(mbus_data_record *record, int record_cnt, int frame_cnt, mbus_data_variable_header *header)
{
    ...
    if (record)
    {
        ...
        timeinfo = gmtime(&rawtime);///h.
        strftime(timestamp,20,"%Y-%m-%dT%H:%M:%S",timeinfo);
        len += snprintf(&buff[len], sizeof(buff) - len, "        <Timestamp GMT>%s</Timestamp GMT>\n", timestamp);
...
mbus_data_variable_record_jSon(mbus_data_record* record, int record_cnt, int frame_cnt, mbus_data_variable_header* header)
{
    ...
    if (record)
    {
        ...
        timeinfo = gmtime(&rawtime);///h.
        strftime(timestamp, 20, "%Y-%m-%dT%H:%M:%S", timeinfo);
        len += snprintf(&buff[len], sizeof(buff) - len, "        \"Timestamp GMT\":  %s\n", timestamp);
...
// jSon h.
//------------------------------------------------------------------------------
/// Return a string containing an j-Son representation of the M-BUS frame data.   h.
//------------------------------------------------------------------------------
...
mbus_data_fixed_jSon(mbus_data_fixed* data)
{
    ...
    if (data)
    {
        ...
        len += snprintf(&buff[len], buff_size - len, "    \"SlaveInformation\":{ \n");
        len += snprintf(&buff[len], buff_size - len, "        \"Id\":  %d\n", (int)mbus_data_bcd_decode(data->id_bcd, 4));
...
mbus_frame_select_secondary_pack(mbus_frame *frame, char *address)
{
    ...
    {
        snprintf(error_str, sizeof(error_str), "%s: frame or address arguments are NULL or invalid.", __FUNCSIG__);
        return -1;
    }
